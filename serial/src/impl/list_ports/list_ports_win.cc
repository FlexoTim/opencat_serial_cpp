...
namespace serial {
	...
	namespace internal {
		...
		const DWORD port_name_max_length{ 256 };
		const DWORD friendly_name_max_length{ 256 };
		const DWORD hardware_id_max_length{ 256 };
	...
	vector<PortInfo> list_ports()
	{
		...
		)
		{
			...
			if (port_name_length > 0
				&& port_name_length <= port_name_max_length)
